...
<body>
	...
	<h1><%= BTUNLP_Titulo %></h1>
    <h2>Agregar Miembro disponible al tipo <c:out value="${tipoItem.getDescripcion()}"/></h2> 
	...
	<table>
				...
				<td class="tabla_centrado">
		    		<select name="nuevoMiembroDispNombre">
		    			<c:forEach var="miembroProyecto" items="${proyecto.getMiembros()}">
			    			<c:if test="${!tipoItem.getMiembrosDisponibles().contains(miembroProyecto)}">
			...
			<tr>
				<td class="tabla_centrado" colspan="2">
					<input type="submit" name="agregarMiembroDisponible" value="Agregar miembro">
				</td>
			</tr>
		</table>
